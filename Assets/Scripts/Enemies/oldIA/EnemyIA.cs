...
public class EnemyIA : MonoBehaviour
{
    ...
    // Controlla se il player è nelle ocndizioni di essere inseguito, aggiornando lo stato di pattuglia/inseguimento e la posizione del player
    void EnviromentView()
    {
        // se il player è nel raggio d'azione del nemico
        ...
        for (int i = 0; i < playerInRange.Length; i++)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToPlayer) < viewAngle / 2)
            {
                ...
                if (!Physics.Raycast(transform.position, dirToPlayer, dstToPlayer, obstacleMask))
                {
                    ...
                    m_IsPatrol = false;             //  smette di pattugliare e va verso il player
                }
                else
                {
                    // se invece il player è nascosto da ostacoli, non succede niente
                    m_playerInRange = false;
                }
            }
